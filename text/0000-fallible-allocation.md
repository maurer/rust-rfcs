...
There are several programming environments where dynamic allocation is considered acceptible, but failure is expected to be handled.
...
* Services which allocate in response to untrused user input
...
Using restricted subsets of features on these crates should only require special setup when developing that crate, not crates which depend on it.
Crates which depend on it will implicitly use all default features of `std` unless otherwise stated, which when unioned with a subset of features will select the default installed `std` crate.
...
- Adds annotations on a large number of functions.
  ...
  This can be partially mitigated by using feature annotations on entire modules until those modules contain some fallible allocation suppporting functions.

# Rationale and alternatives
[rationale-and-alternatives]: #rationale-and-alternatives

## Alternatives
When considering the alternatives, it is imporant to remember a few things:

* This is not a one-off issue - we will likely need to do something similar for `nofp` on platforms which technically support it in the near future.
...
It would be easy to end up with multiple features for sysroot crates which are mutualy contradictory due to using `cfg` tunables instead of features.
...
- What should the default polarity of Cargo's behavior with regards to accepting an overfeatured prebuilt `std` or `alloc` be?
...
This proposal suggests initially masking off nearly all of `std` behind this feature and incrementally bringing functions to availablility without the `infallible_allocation` feature on an as-needed basis under the supervision of the libs team.
...
If this proposal goes through, workspaces for the usecases listed in the motivation section will likely need to explicitly annotate *every* crate in their environment as using restricted features.
